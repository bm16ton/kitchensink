...
void ts_spi_setup(void) {
    ...
    gpio_mode_setup(TS_SPI_PORT, GPIO_MODE_AF, GPIO_PUPD_NONE,
                    TS_CS_PIN | TS_MO_PIN | TS_MI_PIN);
    gpio_set_af(TS_SPI_PORT, GPIO_AF5, TS_CS_PIN | TS_MO_PIN | TS_MI_PIN);
    gpio_set_output_options(TS_SPI_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ,
                            TS_MO_PIN | TS_CS_PIN);

    gpio_mode_setup(ST_CS_PORT, GPIO_MODE_AF, GPIO_PUPD_NONE,
                    TS_SCL_PIN);
    gpio_set_af(ST_CS_PORT, GPIO_AF5, TS_SCL_PIN);
    gpio_set_output_options(ST_CS_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ,
                            TS_SCL_PIN);
