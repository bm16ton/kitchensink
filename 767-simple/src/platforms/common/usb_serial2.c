...
#include "stlinkv3.h"
#include "cdcacm.h"
#define CDCACM_GDB_ENDPOINT CDCACM_GDB_ENDPOINT2
#define CDCACM_UART_ENDPOINT CDCACM_UART_ENDPOINT2
#endif
#include <libopencm3/usb/cdc.h>

static bool slcan_uart_dtr = true;

static void usb_serial_set_state2(usbd_device *dev, uint16_t iface, uint8_t ep);

static enum usbd_request_return_codes slcan_uart_control_request(usbd_device *dev, struct usb_setup_data *req,
	uint8_t **buf, uint16_t *const len, void (**complete)(usbd_device *dev, struct usb_setup_data *req))
{
	...
	case USB_CDC_REQ_SET_CONTROL_LINE_STATE:
		usb_serial_set_state2(dev, req->wIndex, CDCACM_GDB_ENDPOINT);
		slcan_uart_dtr = req->wValue & 1;
	...
	uint8_t **buf, uint16_t *const len, void (**complete)(usbd_device *dev, struct usb_setup_data *req))
{
	...
	switch (req->bRequest) {
	case USB_CDC_REQ_SET_CONTROL_LINE_STATE:
		usb_serial_set_state2(dev, req->wIndex, CDCACM_UART_ENDPOINT);
...
bool slcan_uart_get_dtr(void)
{
	return slcan_uart_dtr;
}

void usb_serial_set_state2(usbd_device *const dev, const uint16_t iface, const uint8_t ep)
{
	uint8_t buf[10];
...
void usb_serial2_set_config(usbd_device *dev, uint16_t value)
{
	usb_config = value;
...
*/
	...
	usbd_register_control_callback(dev,
			...
			slcan_uart_control_request);

	/* Notify the host that DCD is asserted.
	 * Allows the use of /dev/tty* devices on *BSD/MacOS
	 */
	usb_serial_set_state2(dev, SLCAN_IF_NO, CDCACM_SLCAN_ENDPOINT);
	usb_serial_set_state2(dev, UART_IF_NO, CDCACM_UART_ENDPOINT);
}
