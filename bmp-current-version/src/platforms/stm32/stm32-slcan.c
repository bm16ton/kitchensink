...
void CAN_RX0_ISR(void) {
	...
	*p++ = '\r';
	...
	usbd_ep_write_packet(usbdev, CDCACM_SLCAN_ENDPOINT2, buf, p - buf);
}
...
{
	(void)ep;
	usbd_ep_nak_set(dev, CDCACM_SLCAN_ENDPOINT2, 1);
	count_new = usbd_ep_read_packet(dev, CDCACM_SLCAN_ENDPOINT2,
...
void CAN_TX_ISR(void) {
	...
	if (count_new && !count_out) {
		...
		count_out = count_new;
		count_new = 0;
		usbd_ep_nak_set(usbdev, CDCACM_SLCAN_ENDPOINT2, 0);
	}
	...
	*q++ = 0;
	...
	usbd_ep_write_packet(usbdev, CDCACM_SLCAN_ENDPOINT2, txbuf, len);
	if (count_new)
