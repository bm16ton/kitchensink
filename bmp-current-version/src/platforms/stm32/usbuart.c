...
void printuartchar(uint8_t *data) {
char batman[buf_tx_act_sz];
uint8_t i;
    if (pos >= 345) {
        pos = 2;
        pos2 = pos2 + 18;
    ...
    printf("buf_tx_act_sz %d\n", buf_tx_act_sz);
    for(i =0; i < buf_tx_act_sz; i++) {
    sprintf(batman, "%c", data[i]);
...
//        neoeven(0, 10, 150);
	...
	else
	{
		...
		OUTBUFSZ = packet_size;
		printuarthex(packet_buf);
