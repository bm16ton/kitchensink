...
//	    rcc_periph_clock_enable(RCC_GPIOB);
		...
		i2c_reset(I2C3);
		I2C_CR1(I2C3) &=~ I2C_CR1_NOSTRETCH;//disable clock strech
		I2C_CR1(I2C3) &= ~I2C_CR1_ENGC;//diable generaral callback
//		I2C_CR2(I2C3) |= I2C_CR2_LAST;//set next DMA EOT is last transfer
//		i2c_set_dma_last_transfer(I2C3);
		i2c_enable_dma(I2C3);
//		I2C_CR2(I2C3) |=16;//set clock source to 16MHz
//        i2c_set_fast_mode(I2C3);
        i2c_set_clock_frequency(I2C3, 30);
        i2c_set_ccr(I2C3, 30 * 5);
	    i2c_set_trise(I2C3, 30 + 1);
		I2C_CR1(I2C3) |=I2C_CR1_PE;
...
{
	while ((I2C_SR2(I2C3) & I2C_SR2_BUSY)) {
	}

	i2c_send_start(I2C3);
	/* Wait for the end of the start condition, master mode selected, and BUSY bit set */
	while ( !( (I2C_SR1(I2C3) & I2C_SR1_SB)
		&& (I2C_SR2(I2C3) & I2C_SR2_MSL)
		&& (I2C_SR2(I2C3) & I2C_SR2_BUSY) ));

	i2c_send_7bit_address(I2C3, SensorAddr, I2C_WRITE);
  ...
  /* Read SR2 */
	...
	while (!(I2C_SR1(I2C3) & I2C_SR1_BTF))
		;

	if (!(I2C_SR1(I2C3) & I2C_SR1_BTF)) {
		i2c_reset(I2C3);
	}
...
static void DMA_Transmit(uint8_t * pBuffer, uint8_t size)
{
  ...
  if(NULL != pBuffer)
  {
	...
	while((DMA1_S4CR)&DMA_SxCR_EN){;}
        ...
        dma_set_memory_address(DMA1, DMA_STREAM4, (uint32_t) pBuffer);
	    dma_set_number_of_data(DMA1, DMA_STREAM4, size);
