...
{
#if defined(STM32F4) || defined(STM32F7xx) || defined(ARDUINO_ARCH_MBED_RP2040) || defined(ARDUINO_ARCH_RP2040) || defined (TEENSYDUINO)
    prtf(PSTR("STATUS\t\t= 0x%02x RX_DR=%x TX_DS=%x MAX_RT=%x RX_P_NO=%x TX_FULL=%x\r\n"), _status, (_status & _BV(RX_DR)) ? 1 : 0,
...
void RF24::print_byte_register(const char* name, uint8_t reg, uint8_t qty)
{
    ...
    #endif
    #if defined(STM32F4) || defined(STM32F7xx) || defined(ARDUINO_ARCH_MBED_RP2040) || defined(ARDUINO_ARCH_RP2040) || defined (TEENSYDUINO)
    prtf(PSTR(PRIPSTR"\t="),name);
    #else
    printf_P(PSTR(PRIPSTR"\t="),name);
    #endif // !defined(RF24_LINUX)
    #if defined(STM32F4) || defined(STM32F7xx) || defined(ARDUINO_ARCH_MBED_RP2040) || defined(ARDUINO_ARCH_RP2040) || defined (TEENSYDUINO)
    while (qty--) {
        prtf(PSTR(" 0x%02x"), read_register(reg++));
    }
    prtf(PSTR("\r\n"));
    #else
    while (qty--) {
...
void RF24::printDetails(void)
{
    ...
    #endif
    #if defined(STM32F4) || defined(STM32F7xx) || defined(ARDUINO_ARCH_MBED_RP2040) || defined(ARDUINO_ARCH_RP2040) || defined(TEENSYDUINO)
    prtf("================ SPI Configuration ================\n" );
    uint8_t bus_ce = static_cast<uint8_t>(csn_pin % 10);
    uint8_t bus_numb = static_cast<uint8_t>((csn_pin - bus_ce) / 10);
    prtf("CSN Pin\t\t= GPIO %d\n", csn_pin);
    prtf("CE Pin\t\t= Custom GPIO%d\n", ce_pin);
    prtf(PSTR("SPI Speedz\t= %d Mhz\n"), static_cast<uint8_t>(spi_speed / 1000000)); //Print the SPI speed on non-Linux devices
    prtf("================ NRF Configuration ================\n");
    ...
    printf("CSN Pin\t\t= /dev/spidev%d.%d\n", bus_numb, bus_ce);
    printf("CE Pin\t\t= Custom GPIO%d\n", ce_pin);
    printf_P(PSTR("SPI Speedz\t= %d Mhz\n"), static_cast<uint8_t>(spi_speed / 1000000)); //Print the SPI speed on non-Linux
