...
lcd_command(uint8_t cmd, int delay, int n_args, const uint8_t *args)
{
	...
	gpio_clear(GPIOE, GPIO11); /* Select the LCD */
	rx_pend++;
	spi_send(SPI1, cmd);
	...
	if (n_args) {
		gpio_set(GPIOE, GPIO9); /* Set the D/CX pin */
		for (i = 0; i < n_args; i++) {
			rx_pend++;
			spi_send(SPI1, *(args+i));
		}
	...
	gpio_set(GPIOE, GPIO11);	   /* Turn off chip select */
	gpio_clear(GPIOE, GPIO9); /* always reset D/CX */
	if (delay) {
...
//	rcc_periph_clock_enable(RCC_SPI1);
	/* This should configure SPI1 as we need it configured */
	tmp = SPI_SR(LCD_SPI);
