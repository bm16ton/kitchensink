...
static uint8_t trace_rx_buf[NUM_TRACE_PACKETS * TRACE_ENDPOINT_SIZE2];
/* Packet pingpong buffer used for receiving packets */
static uint8_t pingpong_buf[2 * TRACE_ENDPOINT_SIZE2];
/* SWO decoding */
	...
	if (w != r) {
		...
		if (decoding)
			...
			rc = traceswo_decode(dev, CDCACM_UART_ENDPOINT,
								 &trace_rx_buf[r * TRACE_ENDPOINT_SIZE2],
								 TRACE_ENDPOINT_SIZE2);
		else
...
void traceswo_setspeed(uint32_t baudrate)
{
	...
	dma_set_number_of_data(SWO_DMA_BUS, SWO_DMA_STREAM, 2 * TRACE_ENDPOINT_SIZE2);
...
void SWO_DMA_ISR(void)
{
	if (DMA_LISR(SWO_DMA_BUS) & DMA_LISR_HTIF0) {
		...
		memcpy(&trace_rx_buf[w * TRACE_ENDPOINT_SIZE2], pingpong_buf,
			   TRACE_ENDPOINT_SIZE2);
	}
	if (DMA_LISR(SWO_DMA_BUS) & DMA_LISR_TCIF0) {
		DMA_LIFCR(SWO_DMA_BUS) |= DMA_LISR_TCIF0;
		memcpy(&trace_rx_buf[w * TRACE_ENDPOINT_SIZE2],
			   &pingpong_buf[TRACE_ENDPOINT_SIZE2], TRACE_ENDPOINT_SIZE2);
	}
	w = (w + 1) % NUM_TRACE_PACKETS;
	trace_buf_drain(usbdev, TRACE_ENDPOINT2);
}
