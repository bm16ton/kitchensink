...
static enum usbd_request_return_codes gadget0_control_request(
    usbd_device *usbd_dev, struct usb_setup_data *req, uint8_t **buf,
    uint16_t *len,
    void (**complete)(usbd_device *usbd_dev, struct usb_setup_data *req))
{
...
static void cdcacm_set_modem_state(usbd_device *usbd_dev, int iface, bool dsr, bool dcd)
{
	...
	usbd_ep_write_packet(usbd_dev, 0x84 + iface, buf, 10);
}
...
//	(void)wValue;
	...
	usbd_register_control_callback(
		...
		usbd_ep_setup(usbd_dev, 0x03, USB_ENDPOINT_ATTR_BULK, BULK_EP_MAXPACKET,
			gadget0_ss_out_cb);
		usbd_ep_setup(usbd_dev, 0x83, USB_ENDPOINT_ATTR_BULK, BULK_EP_MAXPACKET,
			gadget0_ss_in_cb);
		usbd_register_control_callback(
			usbd_dev,
			USB_REQ_TYPE_VENDOR | USB_REQ_TYPE_INTERFACE,
		...
		gadget0_ss_in_cb(usbd_dev, 0x83);
...
*/	
	...
	usbd_ep_setup(usbd_dev, CDCACM_UART_ENDPOINT | USB_REQ_TYPE_IN,
				  USB_ENDPOINT_ATTR_BULK,
	              CDCACM_PACKET_SIZE, usbuart_usb_in_cb);
	usbd_ep_setup(usbd_dev, (CDCACM_UART_ENDPOINT + 1) | USB_REQ_TYPE_IN,
...
static void send_samples(uint8_t ep)
{
    ...
    if (!is_configured)
        return;
    usbd_ep_write_packet(usbd_dev, ep, &samples, sizeof(samples));
