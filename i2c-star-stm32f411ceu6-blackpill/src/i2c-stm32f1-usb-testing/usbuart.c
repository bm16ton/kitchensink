...
//		nvic_disable_irq(USB_IRQ);
		cnt = usbd_ep_write_packet(usbd_dev, CDCACM_UART_ENDPOINT, data, cnt);
...
{
	if (nvic_get_active_irq(USBUSART_IRQ) || nvic_get_active_irq(USBUSART_DMA_RX_IRQ))
		return 0;
...
//		gpio_toggle(GPIOC, GPIO0);
	...
	else
	{
		...
		if (packet_size)
		{
			const uint16_t written = usbd_ep_write_packet(usbd_dev, 0x83, packet_buf, packet_size);
...
{
//	nvic_disable_irq(USB_IRQ);
//    dma_disable_channel(USBUSART_DMA_BUS, DMA_RX_CHAN);
    dma_clear_interrupt_flags(USBUSART_DMA_BUS, DMA_CHANNEL5, DMA_CGIF);	
    dma_channel_reset(USBUSART_DMA_BUS, DMA_CHANNEL5);	
    dma_channel_reset(USBUSART_DMA_BUS, DMA_CHANNEL5);
    gpio_toggle(GPIOC, GPIO0);
    gpio_toggle(GPIOA, GPIO2);
	/* Enable LED */
...
#define USBUSART_DMA_TX_ISR_TEMPLATE(DMA_TX_CHAN) do {				\
	 ...
	 * Otherwise report trasfer completion.				\
	 */									\
	if (buf_tx_act_sz)							\
	{									\
		usbuart_change_dma_tx_buf();					\
