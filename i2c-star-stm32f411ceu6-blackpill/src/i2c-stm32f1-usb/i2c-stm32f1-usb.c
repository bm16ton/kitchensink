...
const struct usb_endpoint_descriptor adc_endpoint = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
...
//        irqtype = IRQ_TYPE_EDGE_FALLING;
     ...
     else if (req->bRequest == 98) {
        return USBD_REQ_HANDLED;
     }
     else if (req->bRequest == 93) {
        reseten();
        return USBD_REQ_HANDLED;
     }
     else if (req->bRequest == 99) {
      printf("endpoint 0 99 called OK\n");
     gpio_toggle(GPIOC, GPIO0);
     adc_once();
//     bulk_tx_cb(usbd_device * usbd_dev, uint8_t ep)
     return USBD_REQ_HANDLED;
    }
     else if (req->bRequest == 69) {
     printf("endpoint 0 69 called OK\n");
...
static enum usbd_request_return_codes usb_control_adc_request(
    usbd_device *usbd_dev, struct usb_setup_data *req, uint8_t **buf,
    uint16_t *len,
    void (**complete)(usbd_device *, struct usb_setup_data *req))
{
...
static void gpiocb(usbd_device * usbd_dev, uint8_t ep)
{
    static char ret[] = "10.00V";
//    printf("adc_once void\n");
//    printf("adc_once = %ld\n", value);
...
//	while (!adc_eoc(ADC1));
    ...
    usbd_ep_write_packet(usbd_dev, 0x82, ret, sizeof(ret));
}
...
static void usb_set_config(usbd_device *usbd_dev, uint16_t wValue)
{
	...
	usbd_ep_setup(usbd_dev, 0x82, USB_ENDPOINT_ATTR_INTERRUPT, 32, gpiocb);
	...
	usbd_ep_setup(usbd_dev, 0x83, USB_ENDPOINT_ATTR_BULK, 64, bulk_tx_cb);
	
	usbd_register_control_callback(
				usbd_dev,
				USB_REQ_TYPE_VENDOR,
				USB_REQ_TYPE_TYPE,
				usb_control_adc_request);
...
void reseten(void)
{
	...
	OTG_FS_GRSTCTL = OTG_GRSTCTL_TXFFLSH | OTG_GRSTCTL_TXFNUM_ALL | OTG_GRSTCTL_RXFFLSH;
}
...
//	usbd_ep_write_packet(usbd_device usbd_dev, 0x83, buf2, 64);
    usbd_ep_write_packet(usbd_dev, 0x82, buft, sizeof(buft));
    exti_set_trigger(EXTI4, irqtype);
}
...
//    src = (uint16_t)value;
//    src = (uint16_t)value;
//    printf("sent sample value %d\n", src);
    
//    printf("sent sample value %ld\n", src);
//    memset(src, 0, 64);
//    printf("src after memset %d\n", src);
//    printf("sent sample value %d\n", value);
//    if (num_samples == 10) {
//    pp = usbd_ep_write_packet(usbd_dev, 0x83, &ret, sizeof(ret));
//    while (usbd_ep_write_packet(usbd_dev, 0x82, NULL, 0) == 0);
    usbd_ep_write_packet(usbd_dev, 0x82, ret, sizeof(ret));
//    while (usbd_ep_write_packet(usbd_dev, 0x82, NULL, 0) == 0);
/*    test[0] = 1;
    test[1] = 2;
    ...
    test[3] = 1;
    gpio_toggle(GPIOC, GPIO0);
    while (usbd_ep_write_packet(usbd_dev, 0x83, test, sizeof(test)) == 0);
    while (usbd_ep_write_packet(usbd_dev, 0x83, NULL, 0) == 0);
    test2[0] = 1;
    test2[1] = 1;
    test2[2] = 1;
    test2[3] = 1;
  */  
 //   while (usbd_ep_write_packet(usbd_dev, 0x82, test2, sizeof(test2)) == 0);
//    while (usbd_ep_write_packet(usbd_dev, 0x82, NULL, 0) == 0);
//    if (pp) {
//        printf("whent pp\n");
//    };
...
/*    pp2 = usbd_ep_write_packet(usbd_dev, 0x83, NULL, 0);
    if (pp2) {
    printf("whent pp2\n");
    };
 */   
    printf("send_samples ret %s\n", ret);
//    printf("send_samples test %s\n", test);
//    printf("send_samples test2 %s\n", test2);
...
void send_samples(void)
{
    printf("send_samples 4real\n");
    usbd_ep_write_packet(usbd_dev, 0x82, &samples, sizeof(samples));
    printf("send_samples 4real samples output = %s \n", samples);
