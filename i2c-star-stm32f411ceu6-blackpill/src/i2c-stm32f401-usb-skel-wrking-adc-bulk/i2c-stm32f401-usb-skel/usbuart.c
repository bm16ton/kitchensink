...
void usbuart_send_stdout(const uint8_t *data, uint32_t len)
{
	while (len) {
		...
		cnt = usbd_ep_write_packet(usbd_dev, 0x03, data, cnt);
...
//    gpio_toggle(GPIOC, GPIO13);
	usbd_ep_nak_set(dev, 0x03, 1);

	/* Read new packet directly into TX buffer */
	uint8_t *const tx_buf_ptr = &buf_tx[buf_tx_act_idx * TX_BUF_SIZE];
	const uint16_t len = usbd_ep_read_packet(dev, 0x03,
						tx_buf_ptr + buf_tx_act_sz, 64);
...
//            #endif
	...
	if (TX_BUF_SIZE - buf_tx_act_sz >= 64)
		usbd_ep_nak_set(dev, 0x03, 0);
}
#endif
...
//        gpio_clear(GPIOC, GPIO1);
	...
	else
	{
		 ...
		 */
		uint8_t packet_buf[63 + sizeof(uint64_t)];
		uint32_t packet_size;

#ifdef USBUART_DEBUG
		/* Copy data from DEBUG FIFO into local usb packet buffer */
		packet_size = copy_from_fifo(packet_buf, usb_dbg_buf, usb_dbg_out, usb_dbg_in, 63, RX_FIFO_SIZE);
		/* Send if buffer not empty */
		if (packet_size)
		{
			const uint16_t written = usbd_ep_write_packet(usbd_dev, 0x03, packet_buf, packet_size);
		...
		packet_size = copy_from_fifo(packet_buf, buf_rx, buf_rx_out, buf_rx_in, 63, RX_FIFO_SIZE);
...
#define USBUSART_DMA_TX_ISR_TEMPLATE(DMA_TX_CHAN) do {				\
	...
	{									\
		usbuart_change_dma_tx_buf();					\
		usbd_ep_nak_set(usbd_dev, 0x83, 0);		\
	}									\
	else									\
	{									\
