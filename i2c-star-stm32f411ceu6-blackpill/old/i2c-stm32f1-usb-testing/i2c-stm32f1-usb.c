...
void usbuart1_usb_out_cb(usbd_device *usbd_dev, uint8_t ep);
/*
		...
		void (**complete)(usbd_device *dev, struct usb_setup_data *req))
{
	...
	case USB_CDC_REQ_SET_LINE_CODING:
		...
		switch(req->wIndex) {
		case 2:
			usbuart_set_line_coding((struct usb_cdc_line_coding*)*buf, 1);
			return USBD_REQ_HANDLED;
...
static void usbuart_usb_out_cb(int USBUSART, usbd_device *usbd_dev, uint8_t ep, int CDCACM_UART_ENDPOINT)
{
	(void)ep;

	char buf[CDCACM_PACKET_SIZE];
	int len = usbd_ep_read_packet(usbd_dev, CDCACM_UART_ENDPOINT,
...
void usbuart1_usb_out_cb(usbd_device *usbd_dev, uint8_t ep)
{
    usbuart_usb_out_cb(USART1, usbd_dev, ep, 7);
}
...
void tim2_isr(void)
{
	...
	/* process FIFO */
	usbuart_run(TIM2, &buf_rx1_out, &buf_rx1_in, buf_rx1, 7);
}
...
//	(void)wValue;
  ...
  usbd_ep_setup(usbd_dev, 0x87, USB_ENDPOINT_ATTR_BULK, 64, usbuart1_usb_out_cb);
  ...
  usbd_register_control_callback(usbd_dev,
	...
	cdcacm_set_modem_state(usbd_dev, 2, true, true);
